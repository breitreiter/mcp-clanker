...
## Research mode bias toward substrate over source code

- **Symptom.** Asked `imp research --mode=fs` what the `--archive` flag does, 5 minutes after committing it (`87bb841`). Qwen scoped its grep to `imp/` (the substrate dir of markdown docs) instead of the `.cs` source. Found no matches, concluded with "high confidence" that the flag doesn't exist, cited a substrate-design rationale doc about plan-state transitions as the "real" archive concept. Output JSON was structurally clean, content was structurally hallucinated.
- **As the user put it:** *a code researcher that ignores code. powerful strategy.*
- **Suspected cause.** The fs-mode system prompt orients toward the substrate as the source-of-truth surface. Reasonable for substrate-aware questions ("what learnings do we have about X?"), wrong for source-code questions ("what does this flag do?"). Need to read `Prompts/research-fs.md` (or wherever the fs-mode system prompt lives) to confirm where this bias is encoded.
- **Investigative angle.** Run the same question with `--archive` to keep the trace. Read the transcript to see which tools qwen reached for and in what order — probably grep + read_file scoped to `imp/`. The fix is either (a) reword the system prompt to make code vs substrate scoping explicit, (b) add a separate `--mode=src` or similar for code-focused research, or (c) leave the prompt alone but document the failure mode and have callers phrase questions to push at `.cs` files explicitly.
- **Severity.** Not blocking — research is one tool of several, and substrate-focused questions are still its bread and butter. But "fs research silently misreads source-code questions" is a real foot-gun for any future caller (humans or skills) that don't know the bias exists.
- **Adjacent question.** Whether the temp=0 stabilization we applied to triage and locate (`cccacb1`, `7684333`) should also extend to the research executor's categorical decisions (tool-call vs final-answer). Probably yes; not investigated.
