...
public static class Executor
{
        ...
        CancellationToken ct)
    {
        ...
        var tools = Toolbox.Create(workingDirectory, state, sandbox);
        var tracePath = Path.Combine(traceDirectory, "trace.jsonl");
        var transcriptPath = Path.Combine(traceDirectory, "transcript.md");

        var history = new List<ChatMessage>
        {
            new(ChatRole.System, Prompts.LoadSystemPrompt(providerName, contract, sandbox.Mode)),
        ...
        CancellationToken ct)
    {
        ...
        var readOnlyTools = Toolbox.CreateReadOnly(workingDirectory);
