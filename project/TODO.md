...
- Network-egress checker: `curl`/`wget`/`nc`/`ssh`/`scp`/`rsync`/`gh api` (write verbs) denied by default; localhost exempted; per-contract `**Allowed network:**` override *(partial — `NetworkEgressChecker.cs` lands the detection + localhost exemption; RunBash flags `SafetyBreach` with category `rescope_or_capability`. End-to-end validation pending. **`**Allowed network:**` contract override still TODO** — right now an affected contract has no remediation path beyond rewriting to avoid the network call; ship the override before we need to run anything that legitimately hits the network.)*
