...
- Doom-loop detector: N=3 same-tool-args in a row; M=5 consecutive failures; K=2 consecutive denied-network attempts *(N=3 and M=5 shipped in `DoomLoopDetector.cs`; stateful but pure (reads `ExecutorState.RecentCalls`, no side effects). Wired into `Executor.RunAsync` after each tool batch via the same `SafetyBreach` termination path as the pre-flight gates. Trip category: `abandon`. K=2 deliberately deferred — the v1 network gate is a hard block so a single denied attempt already terminates the run; K=2 only becomes reachable if the network gate ever softens. End-to-end validation is harder to engineer than for the pre-flight gates (requires the model to actually loop) — planned via T-004 contract that requests 3 consecutive reads of a non-existent file.)*
