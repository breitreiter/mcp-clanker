...
- ~~**Docker sandbox.** Work-use gate. v1 runs unsandboxed on the host (home only). See `executor-v1-research.md` § "v2 gate: Docker sandbox".~~ *(done 2026-04-23 — v2-plan Phase 6 code shipped. `sandbox/Dockerfile` builds on `mcr.microsoft.com/dotnet/sdk:8.0` with git; `sandbox/build.sh` builds the image + creates the shared `clanker-nuget` Docker volume. `SandboxConfig` record parses from appsettings' new `Sandbox` section. `Tools.BuildBashProcess` branches on `Sandbox.Mode`: Host = `/bin/bash -c` directly, Docker = `docker run --rm --network=none --memory=2g --cpus=2 --pids-limit=256 -v <worktree>:/work -v clanker-nuget:/root/.nuget/packages -w /work <image> /bin/bash -c <command>`. Per-command container (not per-build) for simplicity; overhead amortized by the nuget volume. Default `Mode` is still `Host` for backward compat; flip to `Docker` to opt in. `--network=none` by design: packages already in the cache work, new packages fail to restore — the parent owns package-adoption decisions, clanker never does. Skill + README + build() description all flag this constraint. End-to-end validation pending: adversarial exfil contract that tries `curl https://…` should fail at the egress gate *and* the docker network wall, proving both layers.)*
