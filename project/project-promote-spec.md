...
`<repo>.project-proposals/P-NNN-<slug>.md` describing changes it
...
`/project-promote` is the skill that *reads those proposals, gets
...
```
/project-promote                    # list pending proposals
/project-promote P-NNN              # review and act on one proposal
/project-promote P-NNN --apply      # apply without re-asking (claude-approvable+ only)
/project-promote --batch            # walk through all pending proposals one by one
/project-promote --auto-safe        # auto-apply all always-safe proposals (no prompts)
```
...
Default location: `<repo-parent>/<repo-basename>.project-proposals/`.
...
What `/project-promote` accepts. This is the contract imp must honor
...
`/project-promote P-2026-05-09-001`
```
...
- Concept-page regeneration (touches `concepts/<topic>.md`) — these
  are auto-generated by `/project-sync`, so `/project-promote`
...
```yaml
# proposals: ../<repo>.project-proposals/   # already declared elsewhere
auto_safe: false                              # auto-apply always-safe proposals on /project-promote with no args
