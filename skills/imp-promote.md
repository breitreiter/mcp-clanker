...
- `imp/log.md` is NOT a target for this skill **except for
  `category: migration` proposals** — see "Migration carve-out"
  below. Outside migration, imp writes its own log directly under
  the `imp-gnome` author.
...
| Anything touching `imp/*` (except `category: migration`) | | **out-of-scope — refuse** (imp writes its own dir directly) |
| `create` `imp/learnings/<x>.md` (`category: migration` only) | | **human-required** (see Migration carve-out) |
| `create` `imp/reference/<x>.md` (`category: migration` only) | | **human-required** (see Migration carve-out) |
| `append` `imp/log.md` (`category: migration` only) | | **human-required** (see Migration carve-out) |
| Anything touching `imp/concepts/*` | | **out-of-scope — refuse** (gnome-only auto-generated) |
...
`category: migration` proposals — generated by `/project-migrate` —
are allowed to target `imp/learnings/`, `imp/reference/`, and (via
`append`) `imp/log.md`. The normal "imp writes its own dir directly"
rule doesn't apply: migration is the process of folding legacy
human-authored prose into the substrate, and that legacy content
deserves the same human-review gate as cross-boundary changes.

Carve-out scope:
- `create` to `imp/learnings/<x>.md` — allowed, **human-required**.
- `create` to `imp/reference/<x>.md` — allowed, **human-required**.
- `append` to `imp/log.md` — allowed, **human-required**. The
  cleanup proposal of a migration run typically appends a
  completion record here.
- `imp/concepts/*` — still refused. Concepts are gnome-auto-generated;
  migration never targets them.
- Any change to *existing* `imp/learnings/<x>.md` or
  `imp/reference/<x>.md` (`edit` / `set_frontmatter` on a file
  imp already authored) — still refused. Migration produces new
  entries; updates to gnome-authored entries should go through
  `imp tidy`'s flow.

Commit-author note: cross-boundary changes (`plans/`, `TODO.md`, etc.)
get committed under the user's author. Migration writes to `imp/*`
land under the user's author too — this is a one-shot legacy-ingest
flow, not the steady-state gnome write path. `imp tidy` continues to
use the `imp-gnome` author for its own writes.
...
- Precondition: cross-boundary dirs must be git-clean (no uncommitted
  ...
  separately. **Migration carve-out:** when applying
  `category: migration` proposals, also require
  `imp/learnings/`, `imp/reference/`, and `imp/log.md` to be
  git-clean so the migration writes don't commingle with any
  in-flight gnome work. Rollback path for migration extends to
  `git checkout imp/learnings/ imp/reference/ imp/log.md` plus the
  usual cross-boundary set; orphan new files (uncommitted creates)
  are deleted before checkout.
